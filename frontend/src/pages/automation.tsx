...
import type { AgentDecisionAudit, AgentRunSummary, AssetType, AutomationTask, AutomationTaskConfig, Portfolio, StrategyDeployment } from "@/types";
...
export function AutomationPage() {
  ...
  const refresh = async () => {
    const [nextTask, nextPortfolio, nextRuns, nextDecisions, nextAccounts, deployments] = await Promise.all([
  ...
  const confirmLatestRun = async () => {
    if (!latestRun || !window.confirm("确认按卖出优先顺序提交本次运行的全部待确认模拟订单？")) return;
    setBusy(true);
    try {
      const result = await confirmAutomationRun(accountId, latestRun.run_id);
      setMessage(`已确认 ${result.confirmed.length} 笔模拟订单${result.failures.length ? `，${result.failures.length} 笔失败` : ""}`);
      await refresh();
    } catch (error) {
  ...
  const updateDeploymentStatus = async (status: "active" | "paused" | "archived") => {
    if (!deployment) return;
    if (status === "archived" && !window.confirm("归档后该模拟盘将停止运行，确认继续？")) return;
    setBusy(true);
    try {
      await setStrategyDeploymentStatus(deployment.deployment_id, status);
      setMessage(status === "active" ? "策略部署已启用" : status === "paused" ? "策略部署已暂停" : "策略部署已归档");
      await refresh();
    } catch (error) {
    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          <select className="h-10 rounded-md border bg-background px-3 text-sm" value={accountId} onChange={(event) => navigate(`/automation/${event.target.value}`)}>
            {accounts.map((account) => <option key={account.account_id} value={account.account_id}>{account.name} · {account.account_id}</option>)}
          </select>
      ...
      {deployment && <Card><CardContent className="flex flex-wrap items-center justify-between gap-3 pt-6"><div><p className="font-medium">{deployment.strategy_name} · {deployment.status}</p><p className="text-xs text-muted-foreground">{deployment.deployment_id} · SHA256 {deployment.strategy_sha256.slice(0, 12)}…</p></div><div className="flex gap-2">{deployment.status !== "active" && <Button size="sm" onClick={() => updateDeploymentStatus("active")} disabled={busy}>启用</Button>}{deployment.status === "active" && <Button size="sm" variant="outline" onClick={() => updateDeploymentStatus("paused")} disabled={busy}>暂停</Button>}<Button size="sm" variant="outline" onClick={() => updateDeploymentStatus("archived")} disabled={busy || deployment.status === "archived"}>归档</Button></div></CardContent></Card>}
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.3fr]">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><CalendarClock className="h-4 w-4" />无人值守配置</CardTitle><CardDescription>{accountId} · 所有订单均为模拟订单</CardDescription></CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-2"><Label>执行账户</Label><select disabled={Boolean(config.deployment_id)} className="h-10 w-full rounded-md border bg-background px-3 text-sm disabled:opacity-60" value={config.execution_mode} onChange={(e) => setConfig({ ...config, execution_mode: e.target.value as AutomationTaskConfig["execution_mode"], simulation_only: e.target.value !== "live", live_armed: false })}><option value="paper">paper · 模拟盘</option><option value="live">live · 实盘 Adapter</option></select></div>
            ...
            {config.deployment_id && <p className="rounded-md border bg-muted/40 p-3 text-xs text-muted-foreground">部署 ID：{config.deployment_id}。策略版本、标的池、资产类型和成交参数已锁定；可调整运行时间、模式与风控限额。</p>}
            <div className="space-y-2"><Label>资产类型</Label><select disabled={Boolean(config.deployment_id)} className="h-10 w-full rounded-md border bg-background px-3 text-sm disabled:opacity-60" value={config.asset_type} onChange={(e) => setConfig({ ...config, asset_type: e.target.value as AssetType })}><option value="stock">股票</option><option value="etf">ETF</option><option value="lof">LOF</option></select></div>
            <div className="space-y-2"><Label>交易标的池（逗号分隔）</Label><Input disabled={Boolean(config.deployment_id)} placeholder={config.asset_type === "stock" ? "000001,600519" : config.asset_type === "etf" ? "510300,159915" : "166009"} value={config.universe.join(",")} onChange={(e) => setConfig({ ...config, universe: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })} /></div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2"><Label>策略名称（可选）</Label><Input disabled={Boolean(config.deployment_id)} placeholder="默认路由策略" value={config.strategy_name || ""} onChange={(e) => setConfig({ ...config, strategy_name: e.target.value || null })} /></div>
              <div className="space-y-2"><Label>信号成交</Label><select disabled={Boolean(config.deployment_id)} className="h-10 w-full rounded-md border bg-background px-3 text-sm disabled:opacity-60" value={config.fill_time} onChange={(e) => setConfig({ ...config, fill_time: e.target.value as AutomationTaskConfig["fill_time"] })}><option value="next_open">下一交易日开盘</option><option value="same_close">当日收盘价</option><option value="manual">手动确认</option></select></div>
            </div>
        ...
        <div className="space-y-6">
          <Card><CardHeader><div className="flex items-center justify-between gap-3"><CardTitle className="flex items-center gap-2"><Bot className="h-4 w-4" />Agent 运行记录</CardTitle>{latestRun && decisions.some((item) => item.run_id === latestRun.run_id && item.confirmation_status === "pending") && <Button size="sm" variant="outline" onClick={confirmLatestRun} disabled={busy}>确认本次全部提案</Button>}</div></CardHeader><CardContent><div className="space-y-2">{runs.length === 0 ? <p className="text-sm text-muted-foreground">暂无运行记录</p> : runs.slice(0, 8).map((run) => <div key={run.run_id} className="flex items-center justify-between rounded-md border px-3 py-2 text-sm"><div><div className="font-medium">{run.run_date} · {run.trigger}</div><div className="text-xs text-muted-foreground">{run.symbols_processed}/{run.symbols_total} 标的 · {run.decisions_count} 决策 · {run.orders_count} 订单</div></div><Badge variant={run.status === "completed" ? "success" : run.status === "failed" ? "destructive" : "secondary"}>{statusLabel(run.status)}</Badge></div>)}</div></CardContent></Card>
